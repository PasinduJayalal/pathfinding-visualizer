...
class Main:
    def __init__(self):
        ...
        self.mode = "wall" # default mode is drawing walls
        self.start_pos = None
        self.end_pos = None
        self.path = set()
        self.visited = set()
        ...
        # Initialize buttons
        self.dfs_btn = buttons.Button(self.grid_width + 25, 60, self.dfs_img,0.4)
        self.bfs_btn = buttons.Button(self.grid_width + 25, 100, self.bfs_img,0.4)
        self.dijkstras_btn = buttons.Button(self.grid_width + 25, 140, self.dijkstras_img,0.4)
        self.a_star_btn = buttons.Button(self.grid_width + 25, 180, self.a_star_img,0.4)
        self.clear_btn = buttons.Button(self.grid_width + 25, 280, self.clear_img,0.4)
        ...
        running = True
        while running:
            self.windows.fill((255, 255, 255)) # clear screen
            ...
            # Check button clicks and run corresponding algorithms
    ...
    def redraw(self):
        self.draw_grid()
        pygame.display.update()
        pygame.time.delay(5) # small delay to visualize updates
    
    # Handle mouse inputs for placing walls, start, and end points            
        ...
        if row < self.row and col < self.row:
            if mousebutton[0]:
                if self.mode == "wall":
                    ...
                    self.mode = "wall"
            elif mousebutton[2]: # right-click to remove
    ...
    # Draw the bottom instruction panel and any error messages    
    ...
    # Helper method to render text on screen
    def display(self, text, font, color,x,y):
        img = font.render(text, True, color)
        self.windows.blit(img, (x, y))
    
    # Display current mode (start, end, wall)    
